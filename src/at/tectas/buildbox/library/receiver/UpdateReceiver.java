...
public class UpdateReceiver extends BroadcastReceiver implements
	...
	public void onReceive(Context context, Intent intent) {
		...
		;
		this.parser = new JsonItemParser(this.context, this.helper.deviceModel);
	...
	public void getRomItem() {
		...
		if (!PropertyHelper.stringIsNullOrEmpty(romUrl)) {
			try {
				...
				this.communicator.executeJSONObjectAsyncCommunicator(romUrl,
						this);
			} catch (Exception e) {
				e.printStackTrace();
				wakelock.release();
	...
	public void updateJsonObject(JsonObject result) {
		try {
			...
			String updateInterval = this.pref.getString(this.context
					.getString(R.string.preference_interval_property), null);
			...
			if (updateInterval == null) {
				...
				editor.putString(this.context
						.getString(R.string.preference_interval_property),
						updateInterval);
			...
			if (updateInterval.equals("-1") == false) {
				this.romItem = (DetailItem) this.parser.parseJsonItem(result);

				String lastCheckedVersion = this.pref.getString(this.context
						.getString(R.string.preference_last_checked_version),
						null);
				...
				if (PropertyHelper.stringIsNullOrEmpty(lastCheckedVersion)) {
					...
					editor.putString(
							this.context
									.getString(R.string.preference_last_checked_version),
							this.romItem.version);

					editor.commit();
				}

				if (PropertyHelper.compareVersions(this.romItem.version,
						lastCheckedVersion) != 0) {
					String version = this.helper.getVersion();

					int comparsion = PropertyHelper.compareVersions(version,
							this.romItem.version);
	...
	public void notifyUpdate() {
		...
		builder.setContentTitle(this.context
				.getString(R.string.update_notification_title));

		builder.setContentText(this.context
				.getString(R.string.update_notification_text));
